((Utils) => {
  const App = {
    htmlElements: {
      ...
      babySvg: `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    handlers: {
      pokemonFinderFormOnSubmit: async (e) => {
        ...
        const outputTemplate = App.htmlElements.pokemonFinderOutput;
        ...
        try {
          ...
          const evolutions = (searchType === 'pokemon') ? await Utils.getEvolutionChain(response.species) : '';

          const responseToRender = {...response, ...{evolutions}};    

          const renderedTemplate = App.templates.render({
            searchType,
            responseToRender,
          });
          outputTemplate.innerHTML = renderedTemplate;
        } catch (error) {
          outputTemplate.innerHTML = ` <h1>${error}</h1> `;
        }   
                
        if (App.handlers.isHidden(cleanBtn)) { cleanBtn.classList.remove('hide'); }

        if (App.handlers.isHidden(outputTemplate)) { outputTemplate.classList.remove('hide'); }

      },
      isHidden: (element) => {
        return element.classList.contains('hide');        
      },
      pokemonFinderCleanBtnOnClick : (e) => {
        e.preventDefault();
    ...
    templates: {
      render: ({ searchType, responseToRender }) => {
        ...
        return renderMap[searchType]
          ? renderMap[searchType](responseToRender)
          : App.templates.errorCard();
      },
      errorCard: () => ` <h1>There was an error</h1> `,
      pokemonCard: ({ id, name, weight, height, sprites, abilities, evolutions}) => {
        const spriteSection = App.templates.spriteCard({sprites});
        const weightHeightSection = App.templates.weightHeightCard({weight, height});
        const abilitiesSection = App.templates.abilitiesCard({abilities});
      ...
      spriteCard: ({sprites}) => {
        ...
        return ` <div>
                  ...
                  <div class="clip-wrapper">
                    <div class="clip">
                      <img src="${front}" alt="Front Pokemon Sprite">
                    </div>
                    <div class="clip">
                      <img src="${back}" alt="Back Pokemon Sprite">
                    </div>
                  </div>
                 </div> `;
      },
      ...
      evolutionChainCard: ({evolutions}) => {
        ...
        return ` <div> 
                  <h3>Evolution Chain</h3> ${evolutionList} 
                 </div> `;
      },
    },
  };
